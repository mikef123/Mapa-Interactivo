geocodificadorModulo = (function () {
  ...
  function usaDireccion (direccion, funcionALlamar) {
     ...
     var coordernada = geocodificador.geocode(dir, function(array) {
        var coordenadas = array[0];
        var lat = coordenadas.geometry.viewport.pa.g
        var lng = coordenadas.geometry.viewport.ka.g
        coordernada = new google.maps.LatLng({lat: lat, lng: lng })
        funcionALlamar(dir.address, coordernada)
     });
  }
  ...
  function inicializar () {
    ...
    document.querySelector('#agregar').addEventListener('keypress', function (e) {
      var key = e.which || e.keyCode
      if (key === 13) { // 13 is enter
                // code for enter
